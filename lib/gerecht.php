...
class gerecht
{
    ...
    public function __construct($connection, $usr, $inf, $kte, $ing)
    { ///always public
    ...
    public function selecteerGerecht($gerecht_id = null)
    {
        ...
        while ($row = mysqli_fetch_array($sql_result, MYSQLI_ASSOC)) {
            ...
            $row["waardering"] = $this->calculatewaardering($row["id"]);
    ...
    private function calculatewaardering($gerecht_id)
    {
        $waarderingen = $this->retrieveWaardering($gerecht_id);
        $aantal_waarderingen = count($waarderingen);
        $totaal = 0;
        $i = 0;
        while ($i < $aantal_waarderingen) {
            $totaal = $totaal + $waarderingen[$i]["nummeriekveld"];
            $i++;
        }
        $gemiddelde = $totaal / $aantal_waarderingen;
        return $gemiddelde;
    }
}
